...
%% Color definitions for structure scheme
...
Nowadays there exist a large number of different algorithms. They are divided into different groups by different criteria. Often one algorithm may belong to several groups. Here are some of them \cite{cormen}.
...
Algorithm is considered for practically solvable if its running time is polynomial. It means that the complexity of the algorithm is in $O(n^c)$, $c$ is usually a small constant \cite{cormen}. 
...
\newpage
\section{Sorting algorithms included}

This section describes algorithms included to the software. \cite{cormen}, \citep{alm2}, \cite{knuth3}, \cite{sedgewik} and \cite{geeks} served as sources of information for this section.
...
\begin{enumerate}
 ...
 \item \label{itm:step3} Build max-heap without the nodes on reduced positions
...
\begin{center}
	...
	\includegraphics[scale=0.25]{img/Bucketsort.png}
